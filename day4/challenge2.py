...
def fill_bingo_cards(bingo_cards, bingo_input):
    winning_cards = 0
    latest_winner = 0
    already_won = [False for card in bingo_cards]
    for i in range(len(bingo_input)):
        ...
        if i >=5:
            for j in range(len(bingo_cards)):
                ...
                if result and not already_won[j]:
                    winning_cards += 1
                    already_won[j] = True
                    latest_winner = j
                    print(f"The Latest Winner is {latest_winner}")
                    if winning_cards >= len(bingo_cards):
                        return (bingo_cards[latest_winner], the_num)
